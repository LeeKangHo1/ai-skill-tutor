...
### 사용자 진단
![[Snipaste_2025-11-09_22-18-46.png]]
![[Pasted image 20251109222019.png]]
### 학습 화면
![[Snipaste_2025-11-09_22-25-28.png]]

### 퀴즈

![[Snipaste_2025-11-09_22-27-11.png]]
- 객관식

![[Snipaste_2025-11-09_22-27-44.png]]

![[Snipaste_2025-11-09_22-30-39.png]]
- 주관식

### 채팅으로 실시간 질문
![[Snipaste_2025-11-09_22-35-57.png]]
- 사용자의 채팅 내용을 분석해 다음 학습 과정으로 진행하거나 질문일 경우 질문에 대한 답변을 생성합니다.
### 대시보드
![[Snipaste_2025-11-09_22-38-14.png]]
...
![[Pasted image 20250910092537.png]]
...
- 직접 수집한 양질의 자료들을 벡터 DB에 저장 후 학습 내용 생성이나 사용자 질문에 대한 대답 생성 시 해당 자료를 참고해 좋은 답변을 생성합니다.
---

## 🔧 기술 스택

![[Snipaste_2025-11-09_23-30-40.png]]
...
### 1) 과목
- 수학, 영어
	- 영어의 경우 듀오링고, 산타토익 등 이미 시장에 나온 프로그램이 많음
	- 수학 역시 중, 고등학교 수학 관련 프로그램이 많은 상황 
- 약학
	- 도메인 지식이 존재
	- 실용성(수요) + 자료 접근성의 한계
...
- gpt-4o-mini
	...
	- 구형 모델이지만 여러 개의 에이전트로 협업 시 단독 사용보다 높은 퍼포먼스를 보임
- chromaDB
...
- UI 설계
	...
	- 하나의 챕터 학습 완료 직전 학습 내용, 퀴즈, 피드백, 채팅창 모두를 확인하며 복습 가능
### 4) 문서 작업
...
- 사용자 채팅을 분석하여 벡터 검색용 쿼리를 생성
	...
	- 예시) "트랜스포머와 제프리 힌튼이 뭐에요?" -> "트랜스포머란?" + "제프리 힌튼이 누구야?"
...
- 백엔드 개선
	- 답변의 모든 내용을 생성 후 프론트에 전송하는 것이 아니라 먼저 생성된 내용은 먼저 보내는 streaming 방식 사용
- 프론트엔드 개선
...
- 랭그래프 상태 관리에 대한 이해 부족
	- 개별 에이전트 구현 및 테스트를 완료했으나 랭그래프로 에이전트를 연결하여 통합 테스트 시많은 오류 발생
	- 각 에이전트 코드를 다시 작성하며 많은 시간을 소모 

---
## 4. 프로젝트 소감 
- 개인 프로젝트로 소화하기 힘든 프로젝트 규모였으나 코딩 에이전트를 활용, 프로젝트의 모든 부분을 혼자서 개발이 가능해서 놀라웠습니다.
- 이론 공부를 하면서 부분적으로 구현했던 코드와 실제 프로젝트 코드는 많이 다른 것을 느꼈습니다.
