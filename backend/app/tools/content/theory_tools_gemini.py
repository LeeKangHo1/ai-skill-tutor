...
def theory_generation_tool(
    ...
    """
    AI를 활용한 사용자 맞춤형 이론 설명 대본 생성 (섹션 데이터 버전)
    ...
    try:
        ...
        # 2. 간소화된 프롬프트 생성
        system_instruction, user_prompt = _create_simple_prompts(
            section_data, user_type, is_retry_session
        )
        
        # 3. AI 컨텐츠 생성 (일반 텍스트)
        generated_response = ai_manager.get_text_client(AIProvider.GEMINI).generate_content(
            prompt=user_prompt,
...
def _create_simple_prompts(
    section_data: Dict[str, Any],
    user_type: str,
    is_retry_session: bool
) -> tuple:
    """
    간소화된 시스템 instruction과 사용자 프롬프트 생성
